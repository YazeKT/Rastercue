...
(async()=>{
  ...
  try{
    ...
    const modes=['single-png','single-webp-metadata','batch-jpg','double-jpg','cancel-jpg','native-failure'];
    if(fs.existsSync(path.join(customFolder,'realesr-animevideov3-x2.bin'))){await page.evaluate(({channel,folder})=>window.electron.send(channel,folder),{channel:commands.GET_MODELS_LIST,folder:customFolder});modes.push('custom-anime-2x');}
    for(const mode of modes){
      ...
      if(mode==='custom-anime-2x'){payload.model='realesr-animevideov3-x2';payload.scale='2';payload.saveImageAs='png';}
