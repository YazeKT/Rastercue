...
{
    fprintf(stderr, "Usage: rastercue-cpu -i infile -o outfile [options]...\n\n");
    ...
    fprintf(stderr, "  -t tile-size         CPU tile size (>=32/0=auto, default=200)\n");
    ...
    fprintf(stderr, "  -g gpu-id            unsupported in CPU mode; omit this option\n");
    fprintf(stderr, "  -j load:proc:save    load / NCNN CPU threads / save (default=1:2:2)\n");
    fprintf(stderr, "  -x                   unsupported in this CPU sidecar; use Vulkan for TTA\n");
...
#endif
{
    setlocale(LC_ALL, "");
#if _WIN32
    if (argc == 2 && wcscmp(argv[1], L"--probe-json") == 0)
#else
    if (argc == 2 && strcmp(argv[1], "--probe-json") == 0)
#endif
    {
        printf("{\"schemaVersion\":1,\"backend\":\"cpu\",\"runtime\":\"ncnn\",\"vulkan\":false,\"threads\":%d}\n", std::max(1, ncnn::get_cpu_count()));
        return 0;
    }
    path_t inputpath;
    path_t outputpath;
    ...
    {
        swprintf(parampath, 256, L"%ls/%ls-x%ls.param", model.c_str(), modelname.c_str(), std::to_wstring(scale).c_str());
        swprintf(modelpath, 256, L"%ls/%ls-x%ls.bin", model.c_str(), modelname.c_str(), std::to_wstring(scale).c_str());
    }
    else
    {
    ...
    if (!gpuid.empty())
    {
        fprintf(stderr, "Error: rastercue-cpu does not accept a GPU device. Remove -g.\n");
        return -1;
    }
    if (tta_mode)
    {
    ...
    jobs_proc[0] = std::max(1, std::min(jobs_proc[0], cpu_count));
    const int total_jobs_proc = 1;
...

    {
        ...
        for (int i = 0; i < use_gpu_count; i++)
        {
            realesrgan[i] = new RealESRGAN(gpuid[i], tta_mode, jobs_proc[i]);
