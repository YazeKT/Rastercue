...
5. Before packaging, obtain complete native source and generate notices as below. `npx electron-builder --win --x64 --publish never` produces the installer/ZIP and unpacked app. Publication is a separate explicit owner-authorized step.
...
Verify native-source HEAD is 22774bc42e2bc3c785b5b585d213d960b1348ad5 and submodules match docs/ENGINE-PROVENANCE.md. Archive the complete source recursively, excluding `.git` only, and supply it next to native binary downloads. Public package filters retain only Standard/Digital Art and exclude both Microsoft OpenMP DLLs. Install Microsoft's runtime directly if required. Generated notices are ignored source artifacts. Do not commit the native source checkout to Rastercue.
